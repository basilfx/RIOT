...
static gnrc_pktsnip_t *_send_response_extended(gnrc_pktsnip_t *pkt, knx_apci_extended_t apci, uint8_t *data, size_t len)
{
    ...
    knx_addr_individual_to_str(src_str, &src, sizeof(src_str));
    knx_addr_individual_to_str(dst_str, &dst, sizeof(dst_str));
...
static gnrc_pktsnip_t *_addr_read(knx_device_t *device, gnrc_pktsnip_t *pkt)
{
    ...
    if (!device->info->programming_mode) {
        DEBUG("[gnrc_knx_l7] _addr_read: not in programming mode\n");
        return NULL;
    }
...
static gnrc_pktsnip_t *_addr_write(knx_device_t *device, gnrc_pktsnip_t *pkt)
{
    ...
    DEBUG("[gnrc_knx_l7] _addr_write: addr=%s\n", knx_addr_individual_to_str(addr_str, addr, KNX_ADDR_MAX_STR_LEN));
#endif
...
static gnrc_pktsnip_t *_device_descriptor_read(knx_device_t *device, gnrc_pktsnip_t *pkt)
{
    knx_telegram_t *telegram = pkt->data;

    /* validate request */
    if (knx_telegram_get_payload_length(telegram) != 1) {
        DEBUG("[gnrc_knx_l7_apci] _device_descriptor_read: incorrect payload length\n");
        return NULL;
    }
    ...
    DEBUG("[gnrc_knx_l7] _device_descriptor_read: descriptor=%d\n", descriptor);
    ...
    switch (descriptor) {
        case 0:
            ...
            break;
        default:
            DEBUG("[gnrc_knx_l7] _device_descriptor_read: unhandled\n");
            return NULL;
    }

    return _send_response(pkt, KNX_APCI_DEVICE_DESCRIPTOR_RESPONSE, buf, 3);
}
...
static gnrc_pktsnip_t *_restart(knx_device_t *device, gnrc_pktsnip_t *pkt)
{
    ...
    /* validate request */
    if (knx_telegram_get_payload_length(telegram) != 1) {
        DEBUG("[gnrc_knx_l7_apci] _restart: incorrect payload length\n");
        return NULL;
    }
...
gnrc_pktsnip_t *_handle_apci(knx_device_t *device, gnrc_pktsnip_t *pkt)
{
    ...
    switch (apci) {
        ...
        //    break;
        case KNX_APCI_DEVICE_DESCRIPTOR_READ:
            return _device_descriptor_read(device, pkt);
        // case KNX_APCI_DEVICE_DESCRIPTOR_RESPONSE:
        //    break;
...
static void _send(gnrc_pktsnip_t *pkt)
{
    ...
    if (!gnrc_netapi_dispatch_send(GNRC_NETTYPE_KNX_L4, GNRC_NETREG_DEMUX_CTX_ALL, pkt)) {
        DEBUG("[gnrc_knx_l7] _send: cannot send due to missing transport layer\n");
        gnrc_pktbuf_release(pkt);
    }
}
