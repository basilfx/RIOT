...
static int _addr_get(int argc, char **argv)
{
    ...
    knx_addr_individual_to_str(tmp, &(device.address), sizeof(tmp));
...
static int _addr_set(int argc, char **argv)
{
    ...
    if (knx_addr_individual_from_str(&tmp, argv[3]) == NULL) {
        printf("error: unable to convert address from string (e.g. 1.2.3)\n");
        return 1;
    }

    /* set the address */
    memcpy(&(device.address), &tmp, sizeof(device.address));
...
static int _assoc_list(int argc, char **argv)
{
    ...
    if (device.associations == NULL) {
        puts("error: device does not support associations");
        return 1;
    }
    ...
    if (device.associations->count == 0) {
...
static int _com_list(int argc, char **argv)
{
    ...
    /* check if communication objects are available */
    if (device.com_objects == NULL) {
        puts("error: device does not support communication objects");
        return 1;
    }

    /* list all communication objects */
    if (device.com_objects[0].flags == 0xFF) {
        puts("(no communication objects)");
        return 0;
    }

    for (unsigned i = 0; device.com_objects[i].flags != 0xFF; i++) {
        printf("- %u -> %p (type %d, flags %02x, %d bytes):\n",
               i,
...
static int _mem_list(int argc, char **argv)
{
    ...
    if (device.segments == NULL) {
        puts("error: device does not support memory segments");
        return 1;
    }

    /* list all segments */
    if (device.segments[0].start == 0xFFFF) {
        puts("(no memory segments)");
        return 0;
    }
    ...
    for (unsigned i = 0; device.segments[i].start != 0xffff; i++) {
        ...
        printf("- 0x%04x -> 0x%04x (%d bytes, %s)\n",
...
static int _prop_list(int argc, char **argv)
{
    ...
    if (device.objects == NULL) {
        puts("error: device does not support properties");
        return 1;
    }

    /* list all objects and their properties */
    if (device.objects[0].properties == NULL) {
        puts("(no properties)");
        return 0;
    }
