 ...
 */
#define STORAGE_MAGIC 0x55aa55aa

/**
 * @brief   Holds the storage format version
 */
...
static ssize_t _read_file(char *path, void *dest, size_t len)
{
    ...
    if (size < len) {
        DEBUG("[storage] _read_file: expected %d bytes from '%s', "
              "got %d bytes\n",
              (unsigned)len,
              path,
              (unsigned)size);
...
static ssize_t _write_file(char *path, const void *src, size_t len)
{
    ...
    if (fwrite(src, sizeof(uint8_t), len, f) != len) {
        DEBUG("[storage] _write_file: unable to write to '%s'\n", path);
        fclose(f);
        return -1;
    }
...
int storage_validate(void)
{
    ...
    if (actual_version != STORAGE_VERSION) {
...
int storage_read(void)
{
    ...
    for (unsigned i = 0; segments[i].start != 0xffff; i++) {
        if (segments[i].type == KNX_MEMORY_TYPE_EEPROM) {
            snprintf(path, sizeof(path), "/mtd0/segment%u", i);
