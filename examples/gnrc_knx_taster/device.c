...
static void _buttons_toggle(device_button_t *device_button, bool a, bool b, bool c)
{
    bool value;

    DEBUG("[device] _buttons_toggle: button=%p a=%d b=%d c=%d\n", device_button, (int)a, (int)b, (int)c);
    ...
    if (b) {
        knx_com_object_write(device_button->switch_b, &value, 1);
    }
    if (c) {
        knx_com_object_write(device_button->switch_c, &value, 1);
    }
...
{
    DEBUG("[device] _buttons_dim: button=%p, direction=%d", device_button, direction);
...
static void _cb_buttons_interface(buttons_event_t event, uint8_t repeat, void *arg)
{
    ...
    switch (settings.channels[device_button->channel].function) {
        case DEVICE_BUTTON_FUNCTION_SWITCH:
            _buttons_toggle(device_button, true, false, false);
        ...
        case DEVICE_BUTTON_FUNCTION_INTELLIGENT_DIM:
            if (event == BUTTONS_EVENT_PRESS) {
                if (repeat == 0) {
                    ...
                    _buttons_toggle(device_button, true, true, false);
                }
            }
...
static float _saul_scale_value(float val, int dim)
{
    switch (dim) {
        case -3:
            return val / 1000;
...
static void _cb_sensors(phydat_t *result, void *arg)
{
    ...
    switch (device_sensor->dpt) {
        case KNX_DPT_9:
            value = _saul_scale_value((float)result->val[0], result->scale);
...
{
    // DEBUG("[device] _cb_knx: received event %d with args %p\n", event, args);
    ...
    switch (event) {
        ...
        case KNX_DEVICE_EVENT_SET_ADDRESS:
            ...
            break;
        default:
            DEBUG("[device] _cb_knx: event not handled\n");
    }
}
...
static saul_reg_t* _find_saul_dev(unsigned i)
{
    memory_settings_t *settings = MEMORY_SEGMENT(SETTINGS)->ptr;
    saul_reg_t *reg;

    if (strlen((char *)settings->sensors[i].source) > 0) {
        DEBUG("[device] _find_saul_dev: user-provided type=%d name=%s\n",
              device_sensors[i].saul_type,
              (char *)settings->sensors[i].source);
        reg = saul_reg_find_type_and_name(device_sensors[i].saul_type,
                                          (char *)settings->sensors[i].source);
    }
    else {
        DEBUG("[device] _find_saul_dev: system-provided type=%d name=%s\n",
              device_sensors[i].saul_type,
              device_sensors[i].saul_name);
        reg = saul_reg_find_type_and_name(device_sensors[i].saul_type,
                                          device_sensors[i].saul_name);

        if (reg == NULL) {
            DEBUG("[device] _find_saul_dev: fallback type=%d\n",
                  device_sensors[i].saul_type);
            reg = saul_reg_find_type(device_sensors[i].saul_type);
        }
    }

    if (reg == NULL) {
        DEBUG("[device] _find_saul_dev: no registration found\n");
    }
...
void device_load(void)
{
    ...
    for (unsigned i = 0; i < ARRAY_SIZE(sensors); i++) {
        ...
        sensors[i].saul_dev = _find_saul_dev(i);
        sensors[i].smoothing_algorithm = (sensors_smoothing_algorithm_t)settings->sensors[i].smoothing;
        sensors[i].smoothing_ema_alpha = settings->sensors[i].smoothing_ema_alpha;
        sensors[i].smoothing_ma_samples = settings->sensors[i].smoothing_ma_samples;
    }
