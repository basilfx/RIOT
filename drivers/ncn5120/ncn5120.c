...
int ncn5120_init(ncn5120_t *dev, const ncn5120_params_t *params)
{
    ...
    /* initialize SAVE pin */
    if (gpio_is_valid(dev->params.save)) {
        if (gpio_init_int(dev->params.save, GPIO_IN, GPIO_FALLING, _cb_save, dev) != 0) {
            DEBUG("[ncn5120] ncn5120_init: error initializing SAVE pin\n");
            return NCN5120_ERR_INIT;
        }

        if (dev->params.wait_for_save) {
            DEBUG("[ncn5120] ncn5120_init: waiting for SAVE pin\n");

            for (unsigned i = 0; i < 5; i++) {
                if (gpio_read(dev->params.save) == 1) {
                    break;
                }
            ...
            if (gpio_read(dev->params.save) == 0) {
                DEBUG("[ncn5120] ncn5120_init: SAVE pin still low\n");
                return NCN5120_ERR_INIT;
            }
        }
    }
    ...
    if (uart_init(dev->params.uart_dev, 19200, _cb_uart, dev) != UART_OK) {
        DEBUG("[ncn5120] ncn5120_init: error initializing UART\n");
        return NCN5120_ERR_INIT;
    }

    if (uart_mode(dev->params.uart_dev, UART_DATA_BITS_8, UART_PARITY_EVEN, UART_STOP_BITS_1) != UART_OK) {
        DEBUG("[ncn5120] ncn5120_init: error setting mode\n");
        return NCN5120_ERR_INIT;
    }

    DEBUG("[ncn5120] ncn5120_init: driver initialized\n");
    return NCN5120_OK;
}
...
int ncn5120_busmon(ncn5120_t *dev)
{
    ...
    if (_request(dev, buf, 1, 0) != NCN5120_OK) {
...
int ncn5120_configure(ncn5120_t *dev, uint8_t opts)
{
    ...
    if (_request(dev, buf, 1, 1) != NCN5120_OK) {
        return NCN5120_ERR_REQ;
    }
...
int ncn5120_set_address(ncn5120_t *dev, uint16_t address)
{
    ...
    if (_request(dev, buf, 4, 1) != NCN5120_OK) {
        return NCN5120_ERR_REQ;
    }
...
int ncn5120_set_repetition(ncn5120_t *dev, uint8_t busy_count, uint8_t nack_count)
{
    ...
    if (_request(dev, buf, 4, 0) != NCN5120_OK) {
        return NCN5120_ERR_REQ;
    }
...
int ncn5120_reset(ncn5120_t *dev)
{
    ...
    /* several attempts are necessary to ensure synchronization with the
       transceiver, because a reset on boot might occur in the middle of
       receiving another response, which means the 'response' byte is not the
       expected response byte */
    while (1) {
        if (attempts != 5) {
            DEBUG("[ncn5120] ncn5120_reset: out-of-sync, performing another reset\n");
        }
        ...
        /* request and response are OK, break here */
        break;
    }
...
int ncn5120_get_system_state(ncn5120_t *dev)
{
    ...
    if (_request(dev, buf, 1, 2) != NCN5120_OK) {
        return NCN5120_ERR_REQ;
    }
...
int ncn5120_reg_write(ncn5120_t *dev, uint8_t addr, uint8_t value)
{
    ...
    if (_request(dev, buf, 2, 0) != NCN5120_OK) {
        return NCN5120_ERR_REQ;
    }
...
int ncn5120_send(ncn5120_t *dev, const iolist_t *iolist)
{
    ...
    while (expected) {
        if (isrpipe_read_all_timeout(&(dev->pipe), buf, 1, NCN5120_TIMEOUT_WAIT_FOR_ACK) == -ETIMEDOUT) {
            DEBUG("[ncn5120] ncn5120_send: timeout while waiting for ACK\n");
            break;
        }
