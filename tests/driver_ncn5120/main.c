...
static void _cb(uint8_t event, void *data, void *arg)
{
    ...
    switch (event) {
        case NCN5120_EVENT_TELEGRAM:
            puts("Callback: telegram received");
            break;
        case NCN5120_EVENT_TELEGRAM_INCOMPLETE:
            puts("Callback: incomplete telegram received");
            break;
        case NCN5120_EVENT_STATE:
            printf("Callback: state is 0x%02x\n", *(uint8_t *)data);
            break;
        case NCN5120_EVENT_SYSTEM_STATE:
            printf("Callback: system state is 0x%02x\n", *(uint8_t *)data);
            break;
        case NCN5120_EVENT_SAVE:
            puts("Callback: bus voltage dropping");
            break;
    }
}
