...
static void test_knx_addr_individual__valid(void)
{
    knx_addr_individual_t tmp;

    TEST_ASSERT(knx_addr_individual(&tmp, 1, 1, 8) != NULL);

    TEST_ASSERT(tmp.u8[0] == 0x11);
    TEST_ASSERT(tmp.u8[1] == 0x08);
}

static void test_knx_addr_individual__invalid(void)
{
    knx_addr_individual_t tmp;

    TEST_ASSERT(knx_addr_individual(&tmp, 255, 0, 0) == NULL);
    TEST_ASSERT(knx_addr_individual(&tmp, 0, 255, 0) == NULL);
    TEST_ASSERT(knx_addr_individual(&tmp, 255, 255, 0) == NULL);
}

static void test_knx_addr_individual__null(void)
{
    TEST_ASSERT(knx_addr_individual(NULL, 1, 1, 8) == NULL);
}
...
static void test_knx_addr_individual_from_str__valid(void)
{
    char *addr = "1.1.8";
    knx_addr_individual_t tmp;

    TEST_ASSERT(knx_addr_individual_from_str(&tmp, addr) != NULL);

    TEST_ASSERT(tmp.u8[0] == 0x11);
    TEST_ASSERT(tmp.u8[1] == 0x08);
}

static void test_knx_addr_individual_from_str__invalid(void)
{
    char *addr = "-1.-1.-8";
    knx_addr_individual_t tmp;
...
static void test_knx_addr_individual_from_str__null(void)
{
    char *addr = "1.1.8";

    TEST_ASSERT(knx_addr_individual_from_str(NULL, addr) == NULL);
}

static void test_knx_addr_individual_from_str__null_addr(void)
{
    knx_addr_individual_t tmp;

    TEST_ASSERT(knx_addr_individual_from_str(&tmp, NULL) == NULL);
}

static void test_knx_addr_individual_from_str__incomplete(void)
{
    char *addr = "1.1.";
    knx_addr_individual_t tmp;
...
static void test_knx_addr_individual_from_str__too_long(void)
{
...
static void test_knx_addr_individual_to_str__valid(void)
{
    knx_addr_individual_t addr = { { 0x11, 0x08 } };
    char tmp[KNX_ADDR_MAX_STR_LEN];

    TEST_ASSERT(knx_addr_individual_to_str(tmp, &addr, sizeof(tmp)) != NULL);

    TEST_ASSERT(strcmp(tmp, "1.1.8") == 0);
}

static void test_knx_addr_individual_to_str__too_short(void)
{
    knx_addr_individual_t addr = { { 0x11, 0x08 } };
...
static void test_knx_addr_individual_to_str__null(void)
{
    knx_addr_individual_t addr = { { 0x11, 0x08 } };

    TEST_ASSERT(knx_addr_individual_to_str(NULL, &addr, KNX_ADDR_MAX_STR_LEN) == NULL);
}

static void test_knx_addr_individual_to_str__null_addr(void)
{
    char tmp[KNX_ADDR_MAX_STR_LEN];

    TEST_ASSERT(knx_addr_individual_to_str(tmp, NULL, KNX_ADDR_MAX_STR_LEN) == NULL);
}
...
Test *tests_knx_addr_tests(void)
{
    EMB_UNIT_TESTFIXTURES(fixtures) {
        ...
        new_TestFixture(test_knx_addr_individual__valid),
        new_TestFixture(test_knx_addr_individual__invalid),
        new_TestFixture(test_knx_addr_individual__null),
        ...
        new_TestFixture(test_knx_addr_individual_from_str__valid),
        new_TestFixture(test_knx_addr_individual_from_str__invalid),
        new_TestFixture(test_knx_addr_individual_from_str__null),
        new_TestFixture(test_knx_addr_individual_from_str__null_addr),
        new_TestFixture(test_knx_addr_individual_from_str__incomplete),
        new_TestFixture(test_knx_addr_individual_from_str__too_long),
        ...
        new_TestFixture(test_knx_addr_individual_to_str__valid),
        new_TestFixture(test_knx_addr_individual_to_str__too_short),
        new_TestFixture(test_knx_addr_individual_to_str__null),
        new_TestFixture(test_knx_addr_individual_to_str__null_addr),
