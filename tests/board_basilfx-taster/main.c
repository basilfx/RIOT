...
static int cmd_flash(int argc, char **argv)
{
    ...
    if (res) {
        printf("Unable to initialize flash chip: %d\n", res);
        return 1;
    }

    /* read JDEC ID */
    mtd_jedec_id_t *out = &((mtd_spi_nor_t *)mtd0)->jedec_id;

    printf("Flash Info: bank=%u manufacturer=0x%02x device=0x%02x, 0x%02x\n",
           (unsigned int)out->bank,
           (unsigned int)out->manuf,
           (unsigned int)out->device[0],
           (unsigned int)out->device[1]);
